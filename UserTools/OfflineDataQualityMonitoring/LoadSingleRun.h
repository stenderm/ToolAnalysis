...
public:
    ...
    void extractNtupleInformation(int t_runNumber, const std::vector<std::string> &t_fileNames,
                                  const std::string &t_tankTreeName,
                                  const std::string &t_MRDTreeName, int t_verbosity,
...
private:
    ...
    void loadInformationMRDPhaseIITreeMaker(std::shared_ptr<TFile> t_inputFile, std::string t_MRDTreeName,
                               int t_verbosity,
                               std::unique_ptr<NTupleInformation> &t_ntupleInformationOneRun);
    // checks if tank tree exists and loads the tank ntuple information into a NTupleInformation object
    void loadInformationTankPhaseIITreeMaker(std::shared_ptr<TFile> t_inputFile, std::string t_tankTreeName,
                                int t_verbosity,
                                std::unique_ptr<NTupleInformation> &t_ntupleInformationOneRun);
    ...
    void loadInformationLAPPD(std::shared_ptr<TFile> t_inputFile, std::string t_LAPPDTreeName,
                              int t_verbosity,
                              std::unique_ptr<NTupleInformation> &t_ntupleInformationOneRun);
    // load the run information of the ANNIEEventBuilder format
    void loadInformationANNIEEvent(std::shared_ptr<TFile> t_inputFile, int t_verbosity,
                                   std::unique_ptr<NTupleInformation> &t_ntupleInformationOneRun);
    // open tree
    TTree* openTree(std::shared_ptr<TFile> t_inputFile, std::string treeName, int t_verbosity);
    //
    void addOnePropertyToVector(int t_entry, int t_cluster, double t_propertyToAdd,
