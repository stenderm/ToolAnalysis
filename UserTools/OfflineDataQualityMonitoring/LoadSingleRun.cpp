...
void LoadSingleRun::extractNtupleInformation(int t_runNumber, const std::vector<std::string> &t_fileNames, const std::string &t_tankTreeName, const std::string &t_MRDTreeName, int t_verbosity,
    ...
    for (auto aFileName : t_fileNames) {
        ...
        if (!nTupleFile || nTupleFile->IsZombie()) {
            if (t_verbosity) {
                std::cout << "File with name " << aFileName << " could not have been opened. Skip the file and the run.";
            }
            return;
        }
        ...
        if (aFileName.find("BeamCluster") != std::string::npos) {
            ...
            loadInformationTankPhaseIITreeMaker(nTupleFile, t_tankTreeName, t_verbosity, t_ntupleInformationOneRun);
            loadInformationMRDPhaseIITreeMaker(nTupleFile, t_MRDTreeName, t_verbosity, t_ntupleInformationOneRun);
        }
...
void LoadSingleRun::loadInformationTrigger(std::shared_ptr<TFile> t_inputFile, std::string t_triggerTreeName, int t_verbosity, std::unique_ptr<NTupleInformation> &t_ntupleInformationOneRun) {
...
void LoadSingleRun::loadInformationANNIEEvent(std::shared_ptr<TFile> t_inputFile, int t_verbosity, std::unique_ptr<NTupleInformation> &t_ntupleInformationOneRun) {
    ...
    /// Tank
    ROOT::RDF::RResultPtr<std::vector<int> > numberOfClusters { eventDataFrame.Take<int>("numberOfClusters") };
    ROOT::RDF::RResultPtr<std::vector<std::vector<double> > > clusterCharges { eventDataFrame.Take<std::vector<double> >("clusterCharge") };
    ROOT::RDF::RResultPtr<std::vector<std::vector<double> > > clusterTimes { eventDataFrame.Take<std::vector<double> >("clusterTime") };
    ROOT::RDF::RResultPtr<std::vector<std::vector<double> > > clusterChargesPE { eventDataFrame.Take<std::vector<double> >("clusterPE") };
    ROOT::RDF::RResultPtr<std::vector<std::vector<double> > > clusterChargesMaxPE { eventDataFrame.Take<std::vector<double> >("clusterMaxPE") };
    ROOT::RDF::RResultPtr<std::vector<std::vector<double> > > clusterChargeBalance { eventDataFrame.Take<std::vector<double> >("clusterChargeBalance") };
    ROOT::RDF::RResultPtr<std::vector<int> > numberOfHits { eventDataFrame.Take<int>("nhits") };
    ROOT::RDF::RResultPtr<std::vector<std::vector<int> > > hitTubeIDs { eventDataFrame.Take<std::vector<int> >("hitDetID") };
    ROOT::RDF::RResultPtr<std::vector<std::vector<double> > > hitTimes { eventDataFrame.Take<std::vector<double> >("hitT") };
    ROOT::RDF::RResultPtr<std::vector<std::vector<double> > > hitCharges { eventDataFrame.Take<std::vector<double> >("hitQ") };
    ROOT::RDF::RResultPtr<std::vector<std::vector<double> > > hitChargesPE { eventDataFrame.Take<std::vector<double> >("hitPE") };
    ...
    /// MRD
    ROOT::RDF::RResultPtr<std::vector<int> > numberOfClustersMRD { eventDataFrame.Take<int>("MRDClusterNumber") };
    ROOT::RDF::RResultPtr<std::vector<std::vector<double> > > clusterTimesMRD { eventDataFrame.Take<std::vector<double> >("MRDClusterTime") };
    ROOT::RDF::RResultPtr<std::vector<std::vector<double> > > hitTimesMRD { eventDataFrame.Take<std::vector<double> >("MRDhitT") };
    ROOT::RDF::RResultPtr<std::vector<std::vector<int> > > detectorIDsMRD { eventDataFrame.Take<std::vector<int> >("MRDhitDetID") };
    ROOT::RDF::RResultPtr<std::vector<std::vector<int> > > clusterIDMRD { eventDataFrame.Take<std::vector<int> >("MRDHitClusterIndex") };
    ROOT::RDF::RResultPtr<std::vector<std::vector<int> > > numberOfClusterTracksMRD { eventDataFrame.Take<std::vector<int> >("NumClusterTracks") };
    ROOT::RDF::RResultPtr<std::vector<std::vector<int> > > trackIDMRD { eventDataFrame.Take<std::vector<int> >("MRDClusterIndex") };
    ...
    for (size_t iEvent = 0; iEvent < eventNumbers->size(); ++iEvent) {
        ...
        for (int iCluster = 0; iCluster < numberOfClustersForTriggerTank; iCluster++) {
            ...
            runTankInformation[iEvent].clusterChargePE.push_back((*clusterChargesPE)[iEvent][iCluster]);
            runTankInformation[iEvent].clusterChargeMaxPE.push_back((*clusterChargesMaxPE)[iEvent][iCluster]);
            runTankInformation[iEvent].clusterChargeBalance.push_back((*clusterChargeBalance)[iEvent][iCluster]);
            globalNumberOfClusters++;
        }

        /// Tank Individual tubes
        for(int iHit = 0; iHit < numberOfHits->at(iEvent); iHit++){
            runTankInformation[iEvent].hitsPerTube[hitTubeIDs->at(iEvent).at(iHit)].push_back(std::make_tuple(hitTimes->at(iEvent).at(iHit),
                                                                                                              hitCharges->at(iEvent).at(iHit),
                                                                                                              hitChargesPE->at(iEvent).at(iHit)));

//            std::cout << "Hit Number " << iHit << " on Tube ID " << hitTubeIDs->at(iEvent).at(iHit)
//                      << " time " << hitTimes->at(iEvent).at(iHit) << " charge " << hitCharges->at(iEvent).at(iHit)
//                      << " charge in PE " << hitChargesPE->at(iEvent).at(iHit) << "\n";
        ...
        if(numberOfClustersForTriggerMRD != (*numberOfClusterTracksMRD)[iEvent].size()){
            throw std::invalid_argument("Number of Clusters does not match!");
        }
//        if((*trackIDMRD)[iEvent].size()){
//            std::cout << "Size " << (*trackIDMRD)[iEvent].size() << " eventID " << iEvent << "\n";
//            for(size_t iMRDID = 0; iMRDID < (*trackIDMRD)[iEvent].size(); iMRDID++){
//                std::cout << "MRD Indices " << (*trackIDMRD)[iEvent].at(iMRDID) << " iMRDID " << iMRDID << "\n";
//            }
//            if((*numberOfClusterTracksMRD)[iEvent].size()){
//                std::cout << "Size of MRD Tracks " << (*numberOfClusterTracksMRD)[iEvent].size() << " for event " << iEvent << "\n";
//                for(size_t iMRDID = 0; iMRDID < (*numberOfClusterTracksMRD)[iEvent].size(); iMRDID++){
//                    std::cout << "MRD Tracks " << (*numberOfClusterTracksMRD)[iEvent].at(iMRDID) << " iMRDID " << iMRDID << "\n";
//                }
//            }
//        }
        ...
        for (int iCluster = 0; iCluster < numberOfClustersForTriggerMRD; iCluster++) {
            ...
            numberOfTracksPerRun += (*numberOfClusterTracksMRD)[iEvent][iCluster];
        }

    }
    std::cout << "Number of MRD tracks " << numberOfTracksPerRun << std::endl;
    t_ntupleInformationOneRun->setGlobalMRDTrackNumber(numberOfTracksPerRun);
    ...
    t_ntupleInformationOneRun->setGlobalNumberOfEvents(eventNumbers->size());
    t_ntupleInformationOneRun->setTreeMakerVersion(NTupleInformation::ANNIEEvent);
}

void LoadSingleRun::loadInformationMRDPhaseIITreeMaker(std::shared_ptr<TFile> t_inputFile, std::string t_MRDTreeName, int t_verbosity, std::unique_ptr<NTupleInformation> &t_ntupleInformationOneRun) {
...
void LoadSingleRun::loadInformationTankPhaseIITreeMaker(std::shared_ptr<TFile> t_inputFile, std::string t_tankTreeName, int t_verbosity,
                                                        std::unique_ptr<NTupleInformation> &t_ntupleInformationOneRun) {
...
void LoadSingleRun::loadInformationLAPPD(std::shared_ptr<TFile> t_inputFile, std::string t_LAPPDTreeName, int t_verbosity, std::unique_ptr<NTupleInformation> &t_ntupleInformationOneRun) {
...
TTree* LoadSingleRun::openTree(std::shared_ptr<TFile> t_inputFile, std::string treeName, int t_verbosity) {
    TTree *theTree { dynamic_cast<TTree*>(t_inputFile->Get(treeName.c_str())) };
    if (!theTree) {
        if (t_verbosity) {
            std::cout << "Could not open TTree with name " << treeName << " in file " << t_inputFile->GetName() << ".\n";
        }
        return nullptr;
    }
    return theTree;
